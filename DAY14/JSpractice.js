...
function productPrint( ){ // 함수만들기 , 함수명은 아무거나 , ( ) 매개변수 없는상태 , { } 함수 실행될때 처리할 코드
    ...
    let html = "";// 2. 무엇을 + 배열내 모든 객체들을 TR 구성하여 HTML 만들기 + 반복문 
        for( let index = 0 ; index <= productAry.length-1 ; index++ ){
            ...
            // ccode-->category 변경 : ccdoe의category배열에서 찾기
            let category="";
            for( let index2 = 0 ; index2 <= categoryAry.length-1 ; index2++ ){
                if( product.ccode == categoryAry[index2].ccode ){
                    category = categoryAry[index2].category; //만약 제품의 카테고리가 index번째 카테고리코드와 같으면
                    break;
                }
            }
            html  += `<tr>
...
function productAdd( ){  
    ...
    const obj = { 
        "pcode" : pcode , 
        //URL.createObjectURL(함수명)=>이미지를url로 바꿈
